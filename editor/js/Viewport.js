...
var Viewport = function(editor) {
    ...
    var dynObjects = editor.dynObjects;

    var dynObjectsLight = new THREE.DirectionalLight();
    dynObjectsLight.position.set(5.0, 16.0, 7.5);
    dynObjects.add(dynObjectsLight);
    ...
    this.socket.on('newPositions', function(data) {
        ...
        for (var i = 0; i < data.player.length; i++) {
            if (thisPlayers[data.player[i].number] != undefined) {
                ...
                thisPlayers[data.player[i].number].position.z = data.player[i].y;

                thisPlayers[data.player[i].number].position.y = 5;
            } else {
